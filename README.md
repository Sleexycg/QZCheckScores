...
`Actions` → `I understand my workflows, go ahead and enable them` → `QZCheckScores` → `Run workflow`

### 6. 运行 程序

`Actions` → `QZCheckScores` → `Run workflow`

### 7. 改变查询间隔
项目文件：`.github` → `workflows` → `grade-watch.yml`
第五行` - cron: "*/30 * * * *"`，把30改为其他数字即可（事实上推送间隔非常不稳定，几乎一定会大于这个时间段）


_若你的程序正常运行且未报错，那么在此之后，程序将会每隔 30 分钟(理论上)自动检测一次成绩是否有更新_
...
当修改过Secrets以后最好重新建一个workflow，Github部署稳定性远不如本地部署
